...
```{r}
m <- mascleta%>%select(ends_with("LAeq"))%>%rename(mascleta = "LAeq")%>%pivot_longer(cols="mascleta",names_to= "temporada",values_to = "valores")
f <- fallas%>%select(ends_with("LAeq"))%>%rename(fallas = "LAeq")%>%pivot_longer(cols="fallas",names_to= "temporada",values_to = "valores")
v <- verano%>%select(ends_with("LAeq"))%>%rename(verano = "LAeq")%>%pivot_longer(cols="verano",names_to= "temporada",values_to = "valores")
a <- alarma%>%select(ends_with("LAeq"))%>%rename(alarma = "LAeq")%>%pivot_longer(cols="alarma",names_to= "temporada",values_to = "valores")
n <- navidad%>%select(ends_with("LAeq"))%>%rename(navidad = "LAeq") %>%pivot_longer(cols="navidad",names_to= "temporada",values_to = "valores")
n.a <- no_alarma%>%select(ends_with("LAeq"))%>%rename(no_alarma = "LAeq")%>%pivot_longer(cols="no_alarma",names_to= "temporada",values_to = "valores")
p <- pascua%>%select(ends_with("LAeq"))%>%rename(pascua = "LAeq")%>%pivot_longer(cols="pascua",names_to= "temporada",values_to = "valores")
r <- resto%>%select(ends_with("LAeq"))%>%rename(resto = "LAeq")%>%pivot_longer(cols="resto",names_to= "temporada",values_to = "valores")
...
ggplot(z, aes(x = k, y = valores, color = temporada)) +geom_line()+geom_hline(size=1.5,yintercept = 65) +guides(color = guide_legend(title = "Temporada"))+scale_color_manual(values = c("firebrick1","dodgerblue1",                     "gold","seagreen","brown2","coral1","mediumorchid","cyan3"))
ggplot(z, aes(x = k, y = valores, color = temporada)) +geom_violin()+geom_hline(size=1.5,yintercept = 65) +guides(color = guide_legend(title = "Temporada"))+scale_color_manual(values = c("firebrick1","dodgerblue1",                     "gold","seagreen","brown2","coral1","mediumorchid","cyan3"))
```
