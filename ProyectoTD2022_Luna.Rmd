...
```{r}
table(all_df %>% summarise(maximo_LAeq = max(all_df[,2]),maximo_LAeq_d=max(all_df[,3]),
...
```{r}
table(all_df %>% summarise(minimo_LAeq = min(all_df[,2]),minimo_LAeq_d=min(all_df[,3]),
...
```{r}
table(all_df %>% na.omit %>%summarise(media_LAeq=mean(all_df$LAeq),
...
table(mascleta%>% summarise(media_mascleta_LAeq=mean(mascleta$LAeq),
...
table(fallas%>% summarise(media_fallas_LAeq=mean(fallas$LAeq),
...
table(alarma %>% summarise(media_alarma_LAeq=mean(alarma$LAeq),
...
table(resto %>% summarise(media_resto_LAeq=mean(resto$LAeq),
...
table(no_alarma %>% summarise(media_no_alarma_LAeq=mean(no_alarma$LAeq),
