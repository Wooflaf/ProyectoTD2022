...
A través del paquete `leaflet` podemos visualizar los sensores en su localización correspondiente, así como los servicios de hostelería que hay a su alrededor. Es un gráfico interactivo, aunque es muy aconsejable leer la información del siguiente punto, en las conclusiones.
...
La aplicación está disponible a través del siguiente enlace:

```{r shiny-data}
# Guardamos las variables que serán utilizadas en la aplicación Shiny en un .RData que posteriormente cargaremos.
save(list = c("ids", "all_df", "hosteleria", "all_df_ubi_median"),
